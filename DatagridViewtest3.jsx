...
describe('DataGridView', () => {
  // Define defaultProps to be used in the tests
  const defaultProps = {
    columns: [
  ...
  it('displays the correct rows and columns, handles selection, and calls error functions when needed', async () => {
    render(<DataGridView {...defaultProps} />);  // Use defaultProps for the render
    ...
    fireEvent.click(screen.getByLabelText('Checkbox Select All Rows'));  // Select all rows
    await waitFor(() => {
      expect(defaultProps.dispatchError).toHaveBeenCalledWith('Please select only 10 records');
    });

    // Simulate selecting 10 or fewer rows and clearing the error
    fireEvent.click(screen.getAllByRole('checkbox')[10]); // Deselect the 11th row (selecting 10)
    await waitFor(() => {
